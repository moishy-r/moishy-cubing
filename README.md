...
A TypeScript toolkit for building Rubik's cube speedsolving-method solvers
(CFOP, Roux, ZZ, APB, ...), runnable on Deno or Node, published to JSR and npm.
...
Each package publishes independently. Bump the `version` field in the
package's `deno.json`, merge, then push a tag `<package-dir>-v<version>`
(e.g. `cubing-core-v0.1.0`) to trigger `.github/workflows/release.yml`,
which publishes to both JSR and npm.
