...
```sh
deno add jsr:@moishy/cubing-core   # Deno
npx jsr add @moishy/cubing-core    # Node
```
...
State is stored at the **cubie** level — corner and edge permutation and orientation, plus centre
orientation — so slice moves, wide moves and whole-cube rotations are all first-class, and "are the
centres still where they started?" is a question you can ask.
...
The runner handles strategy racing, phase chaining, cross-step lookahead, colour-neutral orientation
